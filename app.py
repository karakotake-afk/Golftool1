...
# 【TAB 1】 選択された2点を通る理想2次曲線モデル
        with tab1:
            ...
            if not w1_row.empty and not target_row.empty:
                ...
                if x1 != x2:
                    # 1W側（長尺側）での重量変化の物理勾配
                    # 1W側は重量変化を緩やかにするため、通常のX軸（xの増加方向）に対しては
                    # dy/dx を控えめな正の傾き（+2.5 g/inch 付近）に設定します。
                    slope_at_1w = 2.5

                    # y = a*x^2 + b*x + c の係数を求める連立方程式
                    # ① dy/dx = 2*a*x1 + b = slope_at_1w
                    # ② a*(x1^2 - x2^2) + b*(x1 - x2) = y1 - y2
                    # 符号判定による上下逆転を防ぐ確実な展開式:
                    ...
                    # もし万が一 a がマイナス（上に凸の山型）になった場合、
                    # 下に凸（a > 0）を強制担保するための頂点固定2次フィッティングへ補正
                    if a < 0:
                        # 1Wを頂点付近（緩やかな立ち上がり）とする下に凸の放物線: y = a*(x - x1)^2 + y1
                        # 条件: x2（選択番手）で y2 を通る
                        a = (y2 - y1) / ((x2 - x1) ** 2)
                        b = -2 * a * x1
                        c = y1 + a * (x1**2)
